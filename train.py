...
parser.add_argument('--num2', type=int, default=48, help='=num of blocks')
...
config.motion_mlp2.num_layers = args.num2
...
def train_step(h36m_motion_input, h36m_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr):
    ...
    # increment = increment.reshape(b, n, 22, 3).reshape(-1, 3)
    # loss1 = torch.mean(torch.norm(motion_pred - h36m_motion_target, 2, 1))
...
# flops
input_tensor = torch.randn(128, 50, 66)  # batch size=1, 3通道，224x224分辨率
...
#     output = model(input_tensor,input_tensor)  # 执行前向传播
...
with torch.no_grad():
    torch.cuda.synchronize()  # 同步 GPU 以确保准确计时
