import argparse
import os, sys
from scipy.spatial.transform import Rotation as R

import numpy as np
from config import config
from model import siMLPe as Model
from lib.datasets.h36m_eval import H36MEval
from lib.utils.misc import rotmat2xyz_torch, rotmat2euler_torch
# import pandas as pds
import torch
from torch.utils.data import DataLoader
os.environ['CUDA_VISIBLE_DEVICES'] = '2'
results_keys = ['#2', '#4', '#8', '#10', '#14', '#18', '#22', '#25']
# results_keys = ['#2', '#10', '#14', '#25']

def get_dct_matrix(N):
    dct_m = np.eye(N)
    for k in np.arange(N):
        for i in np.arange(N):
            w = np.sqrt(2 / N)
            if k == 0:
                w = np.sqrt(1 / N)
            dct_m[k, i] = w * np.cos(np.pi * (i + 1 / 2) * k / N)
    idct_m = np.linalg.inv(dct_m)
    return dct_m, idct_m

def get_stream(data,view):
    N, T, M =data.shape
    if view=='joint':
        pass
    elif view=='motion':
        motion = np.zeros_like(data)
        motion[:,:-1,:]=data[:,1:,:]-data[:,:-1,:]
        motion[:,-1,:]= motion[:,-2,:]
        data=motion
    return torch.tensor(data)

dct_m, idct_m = get_dct_matrix(config.motion.h36m_input_length_dct)
dct_m = torch.tensor(dct_m).float().cuda().unsqueeze(0)
idct_m = torch.tensor(idct_m).float().cuda().unsqueeze(0)


def regress_pred(model, pbar, num_samples, joint_used_xyz, m_p3d_h36):
    joint_to_ignore = np.array([16, 20, 23, 24, 28, 31]).astype(np.int64)
    joint_equal = np.array([13, 19, 22, 13, 27, 30]).astype(np.int64)

    for (motion_input, motion_target) in pbar:
        motion_input = motion_input.cuda()
        b, n, c, _ = motion_input.shape
        num_samples += b

        motion_input = motion_input.reshape(b, n, 32, 3)
        motion_input = motion_input[:, :, joint_used_xyz].reshape(b, n, -1)
        outputs = []
        step = config.motion.h36m_target_length_train
        if step == 25:
            num_step = 1
        else:
            num_step = 25 // step + 1
        for idx in range(num_step):
            with torch.no_grad():
                if config.deriv_input:
                    motion_input_ = motion_input.clone()
                    motion_input_delta = get_stream(motion_input.cpu(),'motion')
                    motion_input_ = torch.matmul(dct_m[:, :, :config.motion.h36m_input_length], motion_input_.cuda())
                else:
                    motion_input_ = motion_input.clone()
                    motion_input_delta = get_stream(motion_input.cpu(), 'motion')
                output, increment = model(motion_input_, motion_input_delta.cuda())       # model entrence
                # output = model(motion_input_, motion_input_)  # model entrence
                output = torch.matmul(idct_m[:, :config.motion.h36m_input_length, :], output)[:, :step, :]
                # output = output[:, :step, :]
                # output = output
                if config.deriv_output:
                    output = output + motion_input[:, -1:, :].repeat(1, step, 1)

            output = output.reshape(-1, 22 * 3)
            output = output.reshape(b, step, -1)
            outputs.append(output)
            motion_input = torch.cat([motion_input[:, step:], output], axis=1)
        motion_pred = torch.cat(outputs, axis=1)[:, :25]

        motion_target = motion_target.detach()
        b, n, c, _ = motion_target.shape

        motion_gt = motion_target.clone()

        motion_pred = motion_pred.detach().cpu()
        pred_rot = motion_pred.clone().reshape(b, n, 22, 3)
        motion_pred = motion_target.clone().reshape(b, n, 32, 3)
        motion_pred[:, :, joint_used_xyz] = pred_rot

        tmp = motion_gt.clone()
        tmp[:, :, joint_used_xyz] = motion_pred[:, :, joint_used_xyz]
        motion_pred = tmp
        motion_pred[:, :, joint_to_ignore] = motion_pred[:, :, joint_equal]

        #####*********************       visualization        *******************
        # gt_pose_all = motion_gt * 1000
        # pre_pose_all = motion_pred * 1000
        # gt_pose_all = gt_pose_all[30].cpu().detach().numpy().reshape(25, -1)  # short 10,(32*3) long 25,(32*3)
        # pre_pose_all = pre_pose_all[30].cpu().detach().numpy().reshape(25, -1)  # short 10,(32*3) Long 25,(32*3)
        # pred_npz = '/public3/ph/SiMLPe/exps/baseline_h36m/ep_patch/pre_pose/{}_pred.csv'.format('posing30')
        # gt_npz = '/public3/ph/SiMLPe/exps/baseline_h36m/ep_patch/ground_truth/{}_gt.csv'.format('posing30')
        # pre_pose_all = pd.DataFrame(pre_pose_all)
        # gt_pose_all = pd.DataFrame(gt_pose_all)
        # pre_pose_all.to_csv(pred_npz)
        # gt_pose_all.to_csv(gt_npz)


        mpjpe_p3d_h36 = torch.sum(torch.mean(torch.norm(motion_pred * 1000 - motion_gt * 1000, dim=3), dim=2), dim=0)
        m_p3d_h36 += mpjpe_p3d_h36.cpu().numpy()
    m_p3d_h36 = m_p3d_h36 / num_samples
    return m_p3d_h36


def test(config, model, dataloader):
    m_p3d_h36 = np.zeros([config.motion.h36m_target_length])
    titles = np.array(range(config.motion.h36m_target_length)) + 1
    joint_used_xyz = np.array([2, 3, 4, 5, 7, 8, 9, 10, 12, 13, 14, 15, 17, 18, 19, 21, 22, 25, 26, 27, 29, 30]).astype(
        np.int64)
    num_samples = 0

    pbar = dataloader
    m_p3d_h36 = regress_pred(model, pbar, num_samples, joint_used_xyz, m_p3d_h36)

    ret = {}
    for j in range(config.motion.h36m_target_length):
        ret["#{:d}".format(titles[j])] = [m_p3d_h36[j], m_p3d_h36[j]]

    # gt_pose_all = p3d_h36[:, in_n:in_n + out_n]
    # pre_pose_all = p3d_out_6
    # gt_pose_all = gt_pose_all[6].cpu().detach().numpy().reshape(25, -1)  # short 10,(32*3) long 25,(32*3)
    # pre_pose_all = pre_pose_all[6].cpu().detach().numpy().reshape(25, -1)  # short 10,(32*3) Long 25,(32*3)
    # pred_npz = '/home/user0/Public/yyx/pywork/vis/data/pre_pose/{}_pred.csv'.format(action)
    # gt_npz = '/home/user0/Public/yyx/pywork/vis/data/ground_truth/{}_gt.csv'.format(action)
    # pre_pose_all = pd.DataFrame(pre_pose_all)
    # gt_pose_all = pd.DataFrame(gt_pose_all)
    # pre_pose_all.to_csv(pred_npz)
    # gt_pose_all.to_csv(gt_npz)

    return [round(ret[key][0], 2) for key in results_keys], m_p3d_h36.mean()
    # return m_p3d_h36


if __name__ == "__main__":
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('--model-pth', type=str, default=None, help='=encoder path')
    args = parser.parse_args()

    model = Model(config)

    state_dict = torch.load(args.model_pth)  # 读模型
    model.load_state_dict(state_dict, strict=False)  # 加载模型
    model.eval()
    model.cuda()

    config.motion.h36m_target_length = config.motion.h36m_target_length_eval
    dataset = H36MEval(config, 'test')

    shuffle = False
    sampler = None
    train_sampler = None
    dataloader = DataLoader(dataset, batch_size=128,
                            num_workers=1, drop_last=False,
                            sampler=sampler, shuffle=shuffle, pin_memory=True)

    print(test(config, model, dataloader))


